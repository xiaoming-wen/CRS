...
"""
from app.permissions import ROLE_PERMISSIONS, Permission, _role_key_for_permissions
...
def list_effective_permissions_for_role(role: str) -> list[str]:
    ...
    unknown role 视作无权限。
    """
    key = _role_key_for_permissions(role)
    grants = ROLE_PERMISSIONS.get(key) or ROLE_PERMISSIONS.get(role) or ()
